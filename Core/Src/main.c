...
int main(void) {
	...
	while (1) {
		...
		// Catatan: Agar float (%.2f) bekerja di STM32CubeIDE,
		// Anda mungkin perlu mengaktifkan "-u _printf_float" di Project Properties > Settings > MCU Linker.
		// Jika error/kosong, ganti %.2f menjadi %d dan cast ke (int).
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim) {
	if (htim->Instance == TIM2) {
		...
		if (Is_First_Captured == 0) {
			...
			// Kalkulasi Fisika
			if (Difference > 0) {
				// Frekuensi = 1.000.000 / Periode (dalam mikrosekon)
